...
\begin{tikzpicture}
  ...
  \node[draw, fill=color1, below of=env] (ds) {Direct Sensing};
  \node[below=1cm of ds] (nw) {\underline{Network}};
  \node[draw, fill=color1, below of=nw] (if) {Inform};
  \node[draw, fill=color1, below of=if] (es) {External Sensing};
  \node[draw, fill=color1, below of=es] (fb) {Feedback};
  ...
  \node[draw, fill=color1, below of=par] (sh) {Sensor Hardware};
  \node[draw, fill=color1, below of=sh] (ch) {Comm. Hardware};
  \node[below=1cm of ch] (mod) {\underline{Models}};
  \node[draw, fill=color1, below of=mod] (dm) {Data Map};
  \node[draw, fill=color1, below of=dm] (nn) {Neural Network};
  \begin{pgfonlayer}{background}
    \draw[thick,dotted,fill=color5] {
